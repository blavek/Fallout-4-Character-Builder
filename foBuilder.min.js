...
var perks = {
    strength: [
		...
		{name: "Strong Back", 	        levels: [ 2, 10, 20, 30],       ranks: ["What are you, part pack mule? Gain +25 to carry weight.", "You now have +50 to carry weight.", "When overencumbered, you can use Action Points to run.", "When overencumbered, you can fast travel."], tldr: "Carry Weight"},
		{name: "Steady Aim",     	    levels: [ 2, 28],               ranks: ["Stay on target! Hip-fire accuracy is improved when firing any gun.", "Hip-fire accuracy is improved even more when firing any gun."], tldr: "Hip-fire accuracy"},
    ...
    perception: [
        ...
        {name: "Awareness",             levels: [ 2],                   ranks: ["To defeat your enemies, know their weaknesses! You can view a target's specific damage resistances in VATS."], tldr: "See enemy weaknesses"},
        ...
        {name: "Night Person",          levels: [ 2, 25],               ranks: ["You are a creature of the night! Gain +2 to Intelligence and Perception between the hours of 6:00 p.m. and 6:00 a.m.", "You now have +3 Intelligence and Perception between the hours of 6:00 p.m. and 6:00 a.m.", "and night vision when sneaking."], tldr: "Int / Per bonus during hrs: 1800 - 0600 "},
    ...
    endurance: [
        ...
        {name: "Aquaboy",               levels: [ 2, 21],               ranks: ["is your ally. You no longer take radiation damage from swimming, and can breath underwater, You become totally undetectable while submerged."], tldr: "No rads from swimming | breathe | stealth under water"},
        {name: "Rad Resistant",         levels: [ 2, 13, 26],           ranks: ["Exposure to the Wasteland has made you more resilient, instantly granting +10 Radiation Resistance.", "You now have +20 Radiation Resistance.", "You now have +30 Radiation Resistance."], tldr: "Radiation resistance"},
        ...
        {name: "Ghoulish",              levels: [ 2, 24, 48],           ranks: ["Sure, you're still human - on the outside! Radiation now regenerates your lost Health.", "Radiation now regenerates even more of your lost Health.", "Radiation now regenerates even more of your lost Health, and some Feral Ghouls will randomly become friendly."], tldr: "Radiation regenerates health"},
    ...
    charisma: [
        ...
        {name: "Lone Wanderer",         levels: [ 2, 17, 40],           ranks: ["Who needs friends, anyway? When adventuring without a companion, you take 15% less damage and carry weight increases by 50.", "When adventuring without a companion, you take 30% less damage and carry weight increases by 100.", "When adventuring without a companion, you do 25% more damage."], tldr: "Bonuses without companion"},
        {name: "Attack Dog",            levels: [ 2,  9, 25],           ranks: ["Your faithful canine companion can hold an enemy, giving you a greater chance to hit them in V.A.T.S.", "When your dog holds an enemy, there's a chance he'll cripple the limb he's biting.", "When your dog holds an enemy, there's a chance he'll cause them to bleed."], tldr: "Dog can hold, cripple and bleed enemies"},
    ...
    intelligence: [
        {name: "V.A.N.S.",              levels: [ 2],                   ranks: ["Let Vault-Tec guide you! The path to your closest quest target is displayed in V.A.T.S."], tldr: "Improve Pip-Boy map notes"},
        ...
        {name: "Scrapper",              levels: [ 2, 23],               ranks: ["Waste not, want not! You can salvage uncommon components like screws, aluminum and copper when scrapping weapons and armor.", "You can salvage rare components like circuitry, nuclear material, and fiberoptics when scrapping weapons and armor. Items with favorited components are highlighted."], tldr: "Salvage rare components"},
    ...
    agility: [
        ...
        {name: "Action Boy",            levels: [ 2, 18],               ranks: ["There's no time to waste! Your Action Points regenerate 25% faster.", "Your Action Points now regenerate 50% faster."], tldr: "Action Point regeneration"},
        ...
        {name: "Quick Hands",           levels: [ 2, 28],               ranks: ["In combat, there's no time to hesitate. You can reload all guns faster.", "Reloading guns costs no Action Points in VATS."], tldr: "Reload faster and remove AP reload cost"},
    ...
    luck: [
        ...
        {name: "Mysterious Stranger",   levels: [ 2, 22, 41],           ranks: ["Who is he? Why does he help? Who cares! The Mysterious Stranger will appear occasionally in VATS to lend a hand, with deadly efficiency...", "The Mysterious Stranger appears more often in VATS.", "The Mysterious Stranger appears more often in VATS. When he kills an opponent, there is a chance your critical meter gets filled."], tldr: "Stranger appears in V.A.T.S. "},
        ...
        {name: "Critical Banker",       levels: [ 2, 17, 43],           ranks: ["You're a patient battlefield tactician, and can save a Critical Hit, to be used in VATS, when your need it most.", "You can now save 2 Critical Hits, to be used in VATS, when you need them most.", "You can now save 3 Critical Hits, to be used in VATS when you need them most. Banking a Critical has a chance to save an additional Critical."], tldr: "Save Crits in V.A.T.S."},
...
function getPerk(perk, stat) {
    for (var i = 0; i < perk[0].levels.length; i++) {
        character.level[firstUndef(character.level, perk[0].levels[i] + -2)] = perk[0];
    }
...
function calculate() {
    ...
    setTimeout(function() {
        ...
        for (var i = 0; i < inputs.length; i++) {
            character.perkArray[findInPerk(character.perkArray, $(inputs[i]).attr("special"), $(inputs[i]).attr("rank"))].priority = $(inputs[i]).val();
        }
        sortPerkArray();
        buildSpecials();
        buildPerkList();
...
function click(a) {
    ...
    rank = 1 + Number(p[1]);
    if (character.perkArray.length > 0) {
        priority = Number(character.perkArray[character.perkArray.length - 1].priority) + 1
    }
    if (a.checked) {
...
$(document).ready(function() {
    ...
    for (var i = 0; i < perks.strength.length; i++) {
        ...
        temp += "<span class='perk'><label class='checkbox'><input type='checkbox' value='strength," + i + "' /><span></span>&nbsp;" + perks.strength[i].name + "</label></span>";
        temp += "<span class='perk'><label class='checkbox'><input type='checkbox' value='perception," + i + "' /><span></span>&nbsp;" + perks.perception[i].name + "</label></span>";
        temp += "<span class='perk'><label class='checkbox'><input type='checkbox' value='endurance," + i + "' /><span></span>&nbsp;" + perks.endurance[i].name + "</label></span>";
        temp += "<span class='perk'><label class='checkbox'><input type='checkbox' value='charisma," + i + "' /><span></span>&nbsp;" + perks.charisma[i].name + "</label></span>";
        temp += "<span class='perk'><label class='checkbox'><input type='checkbox' value='intelligence," + i + "' /><span></span>&nbsp;" + perks.intelligence[i].name + "</label></span>";
        temp += "<span class='perk'><label class='checkbox'><input type='checkbox' value='agility," + i + "' /><span></span>&nbsp;" + perks.agility[i].name + "</label></span>";
        temp += "<span class='perk'><label class='checkbox'><input type='checkbox' value='luck," + i + "' /><span></span>&nbsp;" + perks.luck[i].name + "</label></span>";
        temp += "</div>";
    }
